...
static void hrtf_tilde_dsp(t_hrtf_tilde *x, t_signal **sp){
  dsp_add(hrtf_tilde_perform,
          5,
          x,
          sp[0]->s_vec,
          sp[1]->s_vec,
  ...
  x->l_buffer = malloc(sizeof(t_float)*x->fft_size);
  x->r_buffer = malloc(sizeof(t_float)*x->fft_size);
...
static void *hrtf_tilde_new(void){
  t_hrtf_tilde *x = (t_hrtf_tilde *)pd_new(hrtf_tilde_class);
...
void hrtf_tilde_free(t_hrtf_tilde *x){
...
void hrtf_tilde_setup(void){
  hrtf_tilde_class = class_new(gensym("hrtf~"),
                                 (t_newmethod)hrtf_tilde_new,
                                 (t_method)hrtf_tilde_free,
                                 sizeof(t_hrtf_tilde),
                                 CLASS_DEFAULT,
                                 0);
